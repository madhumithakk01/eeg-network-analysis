"""
Training and patient-level cross-validation for temporal DL models (Temporal CNN).
"""
...
from .dataset import WindowLevelDataset, collate_patient_batch
...
def evaluate(
    ...
    """Return y_true, y_proba_positive, y_pred (0/1 at 0.5)."""
    model.eval()
    all_probs = []
    all_labels = []
    for seqs, masks, labels in loader:
...
def run_patient_cv(
    ...
    """
    Stratified K-Fold at patient level. Train Temporal CNN per fold; aggregate metrics.
    Sequences are variable length; padding is applied per batch. No truncation.
    """
    ...
    for fold, (train_idx, val_idx) in enumerate(cv.split(patient_ids, y)):
        ...
        train_ds = WindowLevelDataset(
            train_ids, y_train, graph_features_dir,
        )
        val_ds = WindowLevelDataset(
            val_ids, y_val, graph_features_dir,
        )
        g = torch.Generator()
